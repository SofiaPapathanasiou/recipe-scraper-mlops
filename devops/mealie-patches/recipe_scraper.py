...
from mealie.services.scraper.triton_cleaner import clean_with_triton
    ...
    async def scrape(
        ...
        for ScraperClass in self.scrapers:
            ...
            try:
                ...
                continue
            ingredients = " | ".join(
                ing if isinstance(ing, str) else (ing.display or "")
            ...
            recipe_text = f"Title: {recipe.name}\nIngredients: {ingredients}\nInstructions: {instructions}"

            original_note = {"title": "Original Recipe", "text": recipe_text}
            triton_result = clean_with_triton(recipe_text)
            triton_note = {"title": "Triton Cleaned", "text": triton_result or "Triton unavailable"}

            recipe.notes = (recipe.notes or []) + [original_note, triton_note]
